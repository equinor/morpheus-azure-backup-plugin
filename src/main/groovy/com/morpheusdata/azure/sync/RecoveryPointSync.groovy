...
class RecoveryPointSync {
    ...
    private addMissingRecoveryPoints(itemsToAdd, backup, server, inProgressItems, opts) {
        ...
        def filter = "startTime eq '${dateFormat.format(searchStartDate)}' and endTime eq '${dateFormat.format(searchEndDate)}' and operation eq 'Backup' and (backupManagementType eq 'AzureIaasVM' or backupManagementType eq 'AzureWorkload')"
