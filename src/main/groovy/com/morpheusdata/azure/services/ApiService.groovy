...
class ApiService {
    ...
    static listPolicies(Map authConfig, Map opts) {
        ...
        try {
            ...
            def headers = buildHeaders(null, token, opts)
            HttpApiClient.RequestOptions requestOpts = new HttpApiClient.RequestOptions([headers:headers, queryParams: ['api-version': apiVersion, $filter: "backupManagementType eq 'AzureIaasVM' or backupManagementType eq 'AzureWorkload'"]])
    ...
    static listBackupJobs(Map authConfig, Map opts) {
        ...
        try {
            ...
            def apiVersion = '2019-05-13'
            def headers = buildHeaders(null, token, opts)
            def filter = opts.filter ?: "backupManagementType eq 'AzureIaasVM' or backupManagementType eq 'AzureWorkload'"
